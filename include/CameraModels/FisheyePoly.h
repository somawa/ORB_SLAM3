...
namespace ORB_SLAM3 {
    class FisheyePoly : public GeometricCamera {
    ...
    public:
        ...
        FisheyePoly(const std::vector<float> _vParameters, const float _precision) : GeometricCamera(_vParameters),
                                                                                        precision(_precision), mvLappingArea(2,0) {
            assert(mvParameters.size() == 8);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
        FisheyePoly(FisheyePoly* pFisheye) : GeometricCamera(pFisheye->mvParameters), precision(pFisheye->precision), mvLappingArea(2,0) ,tvr(nullptr) {
            assert(mvParameters.size() == 8);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
        ...
        friend std::ostream& operator<<(std::ostream& os, const FisheyePoly& kb);
        friend std::istream& operator>>(std::istream& is, FisheyePoly& kb);
